...
namespace StoreFrontPro.Server
{
   ...
   public class CardModel
   {
      ...
      private static Dictionary<string, List<Tuple<string, string>>> MEMO_COMMON_ATTRS =
      ...
      private static Dictionary<string, List<Tuple<string, List<string>>>> MEMO_ID_OPTIONS =
         new Dictionary<string, List<Tuple<string, List<string>>>>();
      ...
      public void GetImage(Action<BitmapImage> aCallBack)
      {
         if (!m_bCardImageIsLoading)
         {
            ...
            Action<BitmapImage> actionWrapper = 
               (image) => { m_bCardImageIsLoading = false; aCallBack(image); };
