...
std::vector<std::pair<std::string, std::string>>  CopyObject::GetNonUniqueTraits(bool bWithParent) const
{
	...
	lstRetVal = ListHelper::Instance()->ConvertMapToList(NonUniqueTraits);
	if (bWithParent)
	{
...
{
	if (ListHelper::Instance()->List_Find(aszCollectionName, ResidentCollections) == -1)
	{
...
void CopyObject::RemoveMetaTag(std::string aszCollection, std::string aszKey)
{
	...
	{
		int iFound;
		if ((iFound = ListHelper::Instance()->List_Find(aszKey, *targetRemoveList, Config::Instance()->GetTagHelper())) != -1)
		{
...
void CopyObject::SetMetaTag(std::string aszCollection, std::string aszKey, std::string aszVal)
{
	...
	if (IsPerCollectionTag(aszKey))
	{
		...
		{
			int iFind = ListHelper::Instance()->List_Find(aszKey, PerCollectionMetaTags[aszCollection], Config::Instance()->GetTagHelper());
			if (iFind == -1)
			{
	...
	else
	{
		...
		{
			int iFound = ListHelper::Instance()->List_Find(aszKey, MetaTags, Config::Instance()->GetTagHelper());
...
{
	return ListHelper::Instance()->List_Find(aszKey, MetaTags, Config::Instance()->GetTagHelper()) != -1;
}
...
void CopyObject::SetNonUniqueAttr(std::string aszKey, std::string aszValue)
{
	...
	if (lstRestrictions != m_mapNonUniqueAttributesRestrictions->end())
	{
		...
		if ((iIndexOfAllowedTrait = ListHelper::Instance()->List_Find(aszValue, lstRestrictions->second)) != -1)
		{
			// It is a legal value
		...
		while (!bAllSet)
		{
			...
			for (; iter_PairedTraits != m_LstPairedTraits->end(); ++iter_PairedTraits)
			{
				...
				if (pszTargetStr != nullptr &&
					ListHelper::Instance()->List_Find(*pszTargetStr, lstSetTraits) == -1)
				{
					if (bRestrictedMatch)
					{
